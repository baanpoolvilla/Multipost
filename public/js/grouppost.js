...
═══════════════════════════════════════════ */
let _gpPageId   = null;
let _gpGroupIds = new Set();

const PAGES_WITH_GROUPS = (typeof GP_PAGES !== 'undefined'
  ? GP_PAGES.filter(p => (p.groups||[]).filter(g => g.enabled!==false).length > 0)
  : []);

function selectGpPage(btn) {
  document.querySelectorAll('.gp-page-chip').forEach(c => c.classList.remove('active'));
  btn.classList.add('active');
  _gpPageId = btn.dataset.pageId;
  _gpGroupIds.clear();
  renderGpGroups();
  updateGpCountLabel();
}

function renderGpGroups() {
  const container = document.getElementById('gpGroupsContainer');
  if (!container) return;
  const page   = PAGES_WITH_GROUPS.find(p => p.pageId === _gpPageId);
  ...
  container.innerHTML = groups.map(g => `
    <button class="page-sel-chip group-chip active" data-group-id="${g.groupId}" onclick="toggleGpGroup(this)">
      <i class="fa-solid fa-people-group" style="font-size:.6rem;color:#42b72a;flex-shrink:0"></i>
      ${g.groupName}
    </button>`).join('');
  groups.forEach(g => _gpGroupIds.add(g.groupId));
  updateGpCountLabel();
}

function toggleGpGroup(btn) {
  const id = btn.dataset.groupId;
  if (_gpGroupIds.has(id)) { _gpGroupIds.delete(id); btn.classList.remove('active'); }
  else                      { _gpGroupIds.add(id);    btn.classList.add('active'); }
  updateGpCountLabel();
}

function updateGpCountLabel() {
  const el = document.getElementById('gpCountLabel');
  if (el) el.textContent = `เลือกกลุ่ม ${_gpGroupIds.size} กลุ่ม`;
}

/* auto-select the first page on load */
(function () {
  if (!PAGES_WITH_GROUPS.length) return;
  _gpPageId = PAGES_WITH_GROUPS[0].pageId;
  renderGpGroups();
})();
...
async function submitGroupPost() {
  ...
  if (!msg) {
    Swal.fire({ icon: 'warning', title: 'ยังไม่ได้กรอกข้อความ', confirmButtonColor: '#42b72a', confirmButtonText: 'ตกลง' });
    return;
  }
  if (!_gpPageId) {
    Swal.fire({ icon: 'warning', title: 'กรุณาเลือกเพจเพื่อดึง Token', confirmButtonColor: '#42b72a', confirmButtonText: 'ตกลง' });
    return;
  }
  if (_gpGroupIds.size === 0) {
    Swal.fire({ icon: 'warning', title: 'กรุณาเลือกกลุ่มอย่างน้อย 1 กลุ่ม', confirmButtonColor: '#42b72a', confirmButtonText: 'ตกลง' });
    return;
  }

  Swal.fire({
    title: 'กำลังส่งโพสต์กลุ่ม...',
    html: `<i class="fa-solid fa-people-group" style="font-size:2rem;color:#42b72a"></i>
           <p style="margin-top:.7rem;color:#65676b">กำลังโพสต์ไปยัง ${_gpGroupIds.size} กลุ่ม</p>`,
  ...
  fd.append('selectedGroupPage', _gpPageId);
  _gpGroupIds.forEach(id => fd.append('selectedGroups', id));
  selectedFiles.forEach(f => fd.append('images', f));
  _tplLoadedImages.forEach(name => fd.append('templateImages', name));

  try {
    const res  = await fetch('/send-groups', { method: 'POST', body: fd });
  ...
  if (!filtered.length) {
    body.innerHTML = `
      <div style="text-align:center;padding:3rem 0;color:#8a8d91">
        ...
        ${_templates.length === 0
          ? 'ยังไม่มีโพสต์บันทึกไว้<br><small style="font-size:.8rem">กด "สร้างโพสต์" หรือ "บันทึก" เพื่อเริ่มต้น</small>'
          : 'ไม่พบโพสต์ที่ค้นหา'}
      </div>`;
    if (footer) footer.style.display = 'none';
    return;
